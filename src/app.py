import requests
from utils import UtilityClass, SpotifyPlaylistModel, RequestData
...
def get_spotify_playlist_data(playlist_url: str):
    ...
    playlist_id = urlparse(playlist_url).path.split("/")[-1] # Get's the Playlist ID from URL
    playlist_data = requests.request("GET", f"https://api.spotify.com/v1/playlists/{playlist_id}", headers = headers).json()
    music = {}
...
def create_playlist_with_songs(playlist_data: SpotifyPlaylistModel, request_data: RequestData):
    ...
    playlist_id = YTMusic.create_playlist(self=ytmusic, 
    ...
    for song_name, artist_name in playlist_data.items():
        ...
        YTMusic.add_playlist_items(self=ytmusic, playlistId=playlist_id, videoIds=[music_info[0]['videoId']])
